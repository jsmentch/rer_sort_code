...
#csv_in=$1
for csv_in in SRT_unified.csv BLE_unified.csv DEL_unified.csv NWR_unified.csv; do
        # do something with $file

    # open the CSV file
    #head -n 10 SRT_unified.csv | while IFS=',' read -r grade audio_string student_id true_season folder task junk; do
    while IFS=',' read 'year' 'grade' 'audio_string' 'student_id' 'true_season' 'folder' 'task' 'junk'; do
        ...
        #cp $file_in $file_out
        ffmpeg -y -i $file_in -acodec pcm_s16le -ar 16000 $file_out
    #done
    done < $csv_in
done